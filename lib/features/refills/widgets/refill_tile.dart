...
/// A single medicine's supply row with a refill button.
class RefillTile extends ConsumerWidget {
  ...
  Future<void> _refill(BuildContext context, WidgetRef ref) async {
    final newSupply = await showRefillSheet(context, item.med, item.supply);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final countLabel = med.kind == PillKind.capsule
        ? l10n.refillsCapsulesLeft(item.supply)
        : l10n.refillsTabletsLeft(item.supply);
    return Container(
              ...
              children: [
                ...
                Text(
                  ...
                  style: AppText.jakarta(
                    ...
                    color: item.low ? AppColors.alertInk2 : AppColors.muted2,
                  ),
                ),
              ],
            ),
